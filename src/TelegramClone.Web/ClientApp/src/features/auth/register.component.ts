...
@Component({
  ...
  template: `
    <div class="auth-page" #authPage>
      ...
      <div class="auth-card" #authCard>
        ...
        <!-- Form -->
        <form (ngSubmit)="onRegister()" class="auth-form" #authForm>
          <div class="auth-input-group">
            <i class="ph ph-user auth-input-icon"></i>
          ...
          <div class="auth-input-group">
            <i class="ph ph-at auth-input-icon"></i>
          ...
          <button type="submit" [disabled]="isLoading()" class="auth-submit-btn">
            @if (isLoading()) {
              ...
              <i class="ph ph-user-plus" style="font-size: 1.15rem;"></i>
              <span>ثبت‌نام</span>
            }
          </button>
        </form>
        ...
        <div class="auth-footer">
          ...
          <a routerLink="/auth/login" class="auth-link">ورود</a>
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .auth-logo {
      ...
      background: linear-gradient(135deg, #a855f7, var(--tg-primary));
      display: flex; align-items: center; justify-content: center;
...
export class RegisterComponent {
  ...
  async onRegister() {
    ...
    const result = await this.auth.register(this.email, this.password, this.name, this.username || undefined);
